...
public partial class TfEventBusTests : BaseTest
{
	...
	public void TfEventBusTests_Publish_KeyedSubscription_OnlyReceivesMatchingKey()
	{
		using (locker.Lock())
		{
			...
			bus.Publish("USER_LOGIN", new LoginEvent());

			bus.Publish("OTHER_KEY", new LoginEvent());

			bus.Publish(null, new LoginEvent());
	...
	public async Task TfEventBusTests_Publish_GlobalSubscription_ReceivesAllKeysAsync()
	{
		using (await locker.LockAsync())
		{
			...
			await bus.PublishAsync("USER_LOGIN", new LoginEvent());

			await bus.PublishAsync(null, new LoginEvent());
	...
	public void TfEventBusTests_Publish_SpecificTypeSubscription_ReceivesInheritedEvents()
	{
		using (locker.Lock())
		{
			...
			bus.Publish(null, new InheritedLoginEvent());
	...
	public async Task TfEventBusTests_Publish_SpecificTypeSubscription_ReceivesInheritedEventsASync()
	{
		using (await locker.LockAsync())
		{
			...
			await bus.PublishAsync(null, new InheritedLoginEvent());
	...
	public void TfEventBusTests_Publish_InterfaceSubscription_ReceivesAllPayloads()
	{
		using (locker.Lock())
		{
			...
			bus.Subscribe<ITfEventPayload>(mockHandler.Object, null);

			bus.Publish(null, new LoginEvent());
			bus.Publish(null, new ClickEvent());
			bus.Publish(null, new SystemEvent());
	...
	public async Task TfEventBusTests_Publish_InterfaceSubscription_ReceivesAllPayloadsAsync()
	{
		using (await locker.LockAsync())
		{
			...
			await bus.SubscribeAsync<ITfEventPayload>(mockHandler.Object, null);

			await bus.PublishAsync(null, new LoginEvent());
			await bus.PublishAsync(null, new ClickEvent());
			await bus.PublishAsync(null, new SystemEvent());
	...
	public void TfEventBusTests_Publish_NullPayload_CallsHandlerWithNull()
	{
		using (locker.Lock())
		{
			...
			bus.Subscribe<LoginEvent>(p => receivedPayload = p, null);
	...
	public async Task TfEventBusTests_Publish_NullPayload_CallsHandlerWithNullAsync()
	{
		using (await locker.LockAsync())
		{
			...
			await bus.SubscribeAsync<LoginEvent>(p => receivedPayload = p, null);

			await bus.PublishAsync(null, null);
	...
	public void TfEventBusTests_Subscribe_IDisposable_RemovesSubscriptionOnDispose()
	{
		using (locker.Lock())
		{
			...
			var subscription = bus.Subscribe<LoginEvent>(mockHandler.Object, null);
	...
	public async Task TfEventHubTests_SubscribeAsync_IAsyncDisposable_RemovesSubscriptionOnDisposeAsync()
	{
		using (await locker.LockAsync())
		{
			...
			IAsyncDisposable? subscription = await bus.SubscribeAsync<LoginEvent>(mockHandler.Object, null);
