        ...
        : AbstractValidator<TalkChannel>
    {
            ...
            ITfService tfService)
        {
            ...
            RuleSet("general", () =>
            {
                ...
                RuleFor(channel => channel.DataIdentity)
                  .NotEmpty()
                  .WithMessage("The channel data identity is required.");

                RuleFor(channel => channel.CountSharedColumnName)
                  .NotEmpty()
                  .WithMessage("The channel shared column is required.");
            });
            ...
            {
                RuleFor(channel => channel.Id)
                        .Must((channel, id) => { return talkService.GetChannel(id) == null; })
                ...
                RuleFor(channel => channel.DataIdentity)
                        .Must((folder, dataIdentity) =>
                        {
                            if (string.IsNullOrEmpty(dataIdentity))
                                return true;

                            var dataIdentityFound = _tfService.GetDataIdentity(dataIdentity);
                ...
                RuleFor(channel => channel.CountSharedColumnName)
                       .Must((channel, sharedColumnName) =>
                       {
                           ...
                           return sharedColumn.DataIdentity == channel.DataIdentity;
                       })
                       .WithMessage("The selected channel data identity is different to shared column for count data identity.");
            });
...

            {
                RuleFor(channnel => channnel.Id)
                        .Must((channel, id) =>
                        {
                            return talkService.GetChannel(id) != null;
                        })
