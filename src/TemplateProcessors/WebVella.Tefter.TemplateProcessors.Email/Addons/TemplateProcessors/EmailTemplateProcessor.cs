...
public class EmailTemplateProcessor : ITfTemplateProcessorAddon
{
    ...
    public const string DESCRIPTION = "creates emails from a template and data";
    public const string FLUENT_ICON_NAME = "Mail";
    public const TfTemplateResultType RESULT_TYPE = TfTemplateResultType.Email;
        ...
        IServiceProvider serviceProvider)
    {
        ...
        foreach (var item in previewResult.Items)
        {
            ...
            {
                item.Errors.Add(new ValidationError("BccRecipients",
                    "Blind-copy recipients contains invalid email address."));
            }
        ...
        IServiceProvider serviceProvider)
    {
        ...
        foreach (var item in result.Items)
        {
            ...
            emailMessage.RelatedDataSetIds = item.RelatedDatasetIds;
            emailMessage.RelatedSpaceIds = item.RelatedSpaceIds;
        ...
        IServiceProvider serviceProvider)
    {
        ...
        foreach (var attItem in settings.AttachmentItems ?? new List<EmailTemplateSettingsAttachmentItem>())
        {
            ...
            if (((TfTemplate)attTemplate).ContentProcessorType == typeof(ExcelFileTemplateProcessor))
            {
                var attTemplaceSettings =
                    JsonSerializer.Deserialize<ExcelFileTemplateSettings>(attTemplate.SettingsJson);
            ...
            {
                var attTemplaceSettings =
                    JsonSerializer.Deserialize<TextFileTemplateSettings>(attTemplate.SettingsJson);
                var bytes = _tfService.GetBlobByteArray(attTemplaceSettings.TemplateFileBlobId.Value);
        ...
        foreach (var resultItem in tmplResult.ResultItems)
        {
            ...
            emailItem.RelatedRowIds = tfRecordIds;
            emailItem.RelatedDatasetIds = tfDatasetIds;
            emailItem.RelatedSpaceIds = tfSpaceIds;
            try
            {
                ...
                {
                    var tfid = (Guid)row["tf_id"];
                    if (!emailItem.RelatedRowIds.Contains(tfid))
                        emailItem.RelatedRowIds.Add(tfid);
                }

                emailItem.Sender = resultItem.Result.Sender ?? string.Empty;
                emailItem.Recipients = (resultItem.Result.Recipients ?? string.Empty)
                    .Split(";", StringSplitOptions.RemoveEmptyEntries).ToList();
                emailItem.CcRecipients = (resultItem.Result.CcRecipients ?? string.Empty)
                    .Split(";", StringSplitOptions.RemoveEmptyEntries).ToList();
                emailItem.BccRecipients = (resultItem.Result.BccRecipients ?? string.Empty)
                    .Split(";", StringSplitOptions.RemoveEmptyEntries).ToList();
