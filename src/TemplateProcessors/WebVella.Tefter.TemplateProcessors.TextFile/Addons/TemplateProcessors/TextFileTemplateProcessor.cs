    ...
    IServiceProvider serviceProvider)
    {
        ...
        try
        {
                ...
                foreach (var resultItem in tmplResult.ResultItems)
                {
                    ...
                    try
                    {
                        ...
                        if (!String.IsNullOrWhiteSpace(settings.TemplatedFileName))
                        {
                            ...
                            if (fileNameProcessed.ResultItems.Count > 0)
                                resultFileName = fileNameProcessed.ResultItems[0].Result ?? settings.FileName;
                        }
            ...
            else
            {
                ...
                {
                    var name = Path.GetFileNameWithoutExtension(templateFilename);
                    templateFilename = $"{templateFilename}_{filesCounter}{ext}";
                }
