...
public static class EnumExtensions
{
	public static string ToDescriptionString<TEnum>(this TEnum e) where TEnum : IConvertible
	{
		...
		{
			Type type = e.GetType();
			var enumName = type.GetEnumName(e.ToInt32(CultureInfo.InvariantCulture));
			if (enumName is null) return description;			
	...
	{
		if (Enum.TryParse(value.ToString(), out TEnum2 result) && result.ToInt16(null) == value.ToInt16(null))
			return result;
	...
	internal static string GetFluentIcon<TEnum>(this TEnum e) where TEnum : IConvertible
	{
		if (e is Enum)
		{
			Type type = e.GetType();
			var enumName = type.GetEnumName(e.ToInt32(CultureInfo.InvariantCulture));
			if (enumName is null) return "ErrorCircle";
			var memInfo = type.GetMember(enumName);
	...
	{
		TfColorAttribute result =
			new TfColorAttribute(name: "", oklch: "", hex: "", variable: "", number: 0, selectable: false);
		if (e is null)
			return result;
