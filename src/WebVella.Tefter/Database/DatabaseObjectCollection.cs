...
public abstract record DatabaseObjectCollection<T> : IEnumerable<T> where T : DatabaseObject
{
    ...
    internal void Remove(string name)
    {
        ...
        using (_lock.Lock())
        {
            ...
            if (column is null)
                throw new DatabaseException($"Trying to remove non existent {typeof(T)} with '{name}' from table");
