...
public abstract record TfDatabaseObjectCollection<T> : IEnumerable<T> where T : TfDatabaseObject
{
    ...
    public TfDatabaseObjectCollection()
    {
        _lock = new AsyncLock();
    ...
    internal void Remove(string name)
    {
        ...
        using (_lock.Lock())
        {
            ...
            if (column is null)
                throw new TfDatabaseException($"Trying to remove non existent {typeof(T)} with '{name}' from table");
