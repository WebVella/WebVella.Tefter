...
public partial class TucLongIntegerDisplayColumnComponent : TucBaseViewColumn<TucLongIntegerDisplayColumnComponentOptions>
{
	...
	public TucLongIntegerDisplayColumnComponent()
	{
	}
	...
	public TucLongIntegerDisplayColumnComponent(TfSpaceViewColumnScreenRegionContext context)
	{
	...
	public override void ProcessExcelCell(IServiceProvider serviceProvider, IXLCell excelCell)
	{
		...
		else if (_value.Count == 1)
		{
			...
			excelCell.SetValue(XLCellValue.FromObject((long?)_value[0]));
		}
		else
		{
		...
		if (column.IsJoinColumn)
		{
			if (columnData.GetType().ImplementsInterface(typeof(IList)))
			{
				foreach (var joinValue in columnData as IList)
					_value.Add((long?)joinValue);
			}
		}
		else
			_value.Add((long?)columnData);
	}
	#endregion
}
...
/// from the database
/// </summary>
public class TucLongIntegerDisplayColumnComponentOptions
{
