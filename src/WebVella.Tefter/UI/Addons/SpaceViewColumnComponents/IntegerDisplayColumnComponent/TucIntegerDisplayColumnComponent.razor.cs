...
[LocalizationResource("WebVella.Tefter.Web.Addons.SpaceViewColumnComponents.IntegerDisplayColumnComponent.TucIntegerDisplayColumnComponent", "WebVella.Tefter")]
public partial class TucIntegerDisplayColumnComponent : TucBaseViewColumn<TucIntegerDisplayColumnComponentOptions>
{
	...
	public TucIntegerDisplayColumnComponent()
	{
	}
	...
	public TucIntegerDisplayColumnComponent(TfSpaceViewColumnScreenRegionContext context)
	{
	...
	public override void ProcessExcelCell(IServiceProvider serviceProvider, IXLCell excelCell)
	{
		...
		else if (_value.Count == 1)
		{
			...
			excelCell.SetValue(XLCellValue.FromObject((int?)_value[0]));
		}
		else
		{
		...
		if (column.IsJoinColumn)
		{
			if (columnData.GetType().ImplementsInterface(typeof(IList)))
			{
				foreach (var joinValue in columnData as IList)
					_value.Add((int?)joinValue);
			}
		}
		else
			_value.Add((int?)columnData);
	}
	#endregion
}
...
/// from the database
/// </summary>
public class TucIntegerDisplayColumnComponentOptions
{
