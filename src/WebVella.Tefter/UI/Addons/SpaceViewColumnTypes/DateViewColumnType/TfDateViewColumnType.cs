...
public class TfDateViewColumnType : ITfSpaceViewColumnTypeAddon
{
	...
	public void ProcessExcelCell(TfSpaceViewColumnBaseContext args)
	{
		...
		{
			var settings = context.GetColumnTypeOptions(new TfDateViewColumnTypeSettings());
	...
	//Value
	private List<DateOnly?> _initValue(TfSpaceViewColumnBaseContext args)
	{
	...
	private RenderFragment _renderReadMode(TfSpaceViewColumnReadModeContext context)
	{
		...
		return builder =>
		{
			...
			builder.AddAttribute(2, "Settings", context.GetColumnTypeOptions(new TfDateViewColumnTypeSettings()));
	...
	private RenderFragment _renderEditMode(TfSpaceViewColumnEditModeContext context)
	{
		    ...
		    || column.Origin == TfDataColumnOriginType.SharedColumn)
		{
			...
			return builder =>
			{
				...
				builder.AddAttribute(2, "ValueChanged", EventCallback.Factory.Create<DateOnly?>(this, async (x) =>
				{
				...
				builder.AddAttribute(3, "Settings", context.GetColumnTypeOptions(new TfDateViewColumnTypeSettings()));
				builder.CloseComponent();
			};
		}
	...
	private RenderFragment _renderOptionsMode(TfSpaceViewColumnOptionsModeContext context)
	{
		return builder =>
		{
			...
			builder.AddAttribute(1, "Settings", context.GetColumnTypeOptions(new TfDateViewColumnTypeSettings()));
			builder.AddAttribute(2, "ValidationErrors", context.ValidationErrors);
			builder.AddAttribute(4, "SettingsChanged", EventCallback.Factory.Create<TfDateViewColumnTypeSettings>(this,
				async (options) =>
				{
