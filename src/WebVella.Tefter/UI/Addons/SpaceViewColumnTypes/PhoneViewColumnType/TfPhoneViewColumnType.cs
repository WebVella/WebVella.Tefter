...
public class TfPhoneViewColumnType : ITfSpaceViewColumnTypeAddon
{
	...
	private RenderFragment _renderReadMode(TfSpaceViewColumnReadModeContext context)
	{
		...
		return builder =>
		{
			...
			builder.AddAttribute(2, "Settings", context.GetColumnTypeOptions(new TfPhoneViewColumnTypeSettings()));
	...
	private RenderFragment _renderEditMode(TfSpaceViewColumnEditModeContext context)
	{
		    ...
		    || column.Origin == TfDataColumnOriginType.SharedColumn)
		{
			...
			return builder =>
			{
				...
				builder.AddAttribute(3, "Settings", context.GetColumnTypeOptions(new TfPhoneViewColumnTypeSettings()));
				builder.CloseComponent();
			};
		}
	...
	private RenderFragment _renderOptionsMode(TfSpaceViewColumnOptionsModeContext context)
	{
		return builder =>
		{
			...
			builder.AddAttribute(1, "Settings", context.GetColumnTypeOptions(new TfPhoneViewColumnTypeSettings()));
			builder.AddAttribute(2, "ValidationErrors", context.ValidationErrors);
			builder.AddAttribute(4, "SettingsChanged", EventCallback.Factory.Create<TfPhoneViewColumnTypeSettings>(this, async (options) =>
			{
