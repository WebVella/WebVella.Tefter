window.Tefter = {
    ...
    dispose: function () {
        ...
        Tefter.ColumnResizeListeners = {};
        Tefter.ColumnSortListeners = {};
    ...
    removeGlobalSearchKeyListener: function (listenerId) {
        if (Tefter.GlobalSearchListeners[listenerId]) {
            delete Tefter.GlobalSearchListeners[listenerId];
        }
        return true;
    },
    executeGlobalSearchKeyListenerCallbacks: function () {
        if (Tefter.GlobalSearchListeners) {

            for (const prop in Tefter.GlobalSearchListeners) {
                const dotNetHelper = Tefter.GlobalSearchListeners[prop].dotNetHelper;
                const methodName = Tefter.GlobalSearchListeners[prop].methodName;
                if (dotNetHelper && methodName) {
                    dotNetHelper.invokeMethodAsync(methodName);
                }
            }
    ...
    removeArrowsKeyListener: function (listenerId) {
        if (Tefter.ArrowsListeners[listenerId]) {
            delete Tefter.ArrowsListeners[listenerId];
        }
        return true;
    },
    executeArrowsListenerCallbacks: function (isUp) {
        if (Tefter.ArrowsListeners) {

            for (const prop in Tefter.ArrowsListeners) {
                const dotNetHelper = Tefter.ArrowsListeners[prop].dotNetHelper;
                const methodName = Tefter.ArrowsListeners[prop].methodName;
                if (dotNetHelper && methodName) {
                    dotNetHelper.invokeMethodAsync(methodName, isUp);
                }
            }
        }
        return true;
    ...
    removeEnterKeyListener: function (listenerId) {
        if (Tefter.EnterKeyListeners[listenerId]) {
            delete Tefter.EnterKeyListeners[listenerId];
        }
        return true;
    },
    executeEnterKeyListenerCallbacks: function () {
        if (Tefter.EnterKeyListeners) {

            for (const prop in Tefter.EnterKeyListeners) {
                const dotNetHelper = Tefter.EnterKeyListeners[prop].dotNetHelper;
                const methodName = Tefter.EnterKeyListeners[prop].methodName;
                if (dotNetHelper && methodName) {
                    dotNetHelper.invokeMethodAsync(methodName);
                }
            }
    ...
    scrollToElement: function (elementId) {
        ...
        element.scrollIntoView({
            behavior: 'smooth', // Use 'smooth' for a scrolling animation, or 'auto' for an instant jump
            block: 'nearest',   // Ensure the element is visible: 'start', 'center', 'end', or 'nearest'
            inline: 'nearest'   // Ensure horizontal visibility
        });
        return true;
    },
}
...
document.addEventListener("keydown", function (evtobj) {
    if (evtobj.key === "ArrowUp" || evtobj.key === "ArrowDown") {
        if (Tefter.ArrowsListeners && Object.keys(Tefter.ArrowsListeners).length > 0) {
            evtobj.stopPropagation();
            evtobj.preventDefault();
