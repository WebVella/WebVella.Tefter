...
public class TfDataProviderColumn
{
		...
		string? expressionJson = null )
	{
		switch (ruleSet)
		{
			...
			case TfDataProviderColumnRuleSet.NotNullableWithCustomDefault:
				IsNullable = false;
				IsUnique = false;
				AutoDefaultValue = false;
			...
			case TfDataProviderColumnRuleSet.NotNullableWithAutoDefault:
				IsNullable = false;
				IsUnique = false;
			...
			case TfDataProviderColumnRuleSet.UniqueNotNullable:
				IsNullable = false;
				IsUnique = true;
				AutoDefaultValue = true;
				DefaultValue = null;
	...
	public TfDataProviderColumnRuleSet GetRuleSet()
	{
		...
		if (!IsNullable && !IsUnique && !AutoDefaultValue && DefaultValue is not null)
			return TfDataProviderColumnRuleSet.NotNullableWithCustomDefault;

		//3. IsNullable = false, IsUnique = false, AutoDefaultValue = true, DefaultValue = null
		if (!IsNullable && !IsUnique && AutoDefaultValue && DefaultValue is null)
			return TfDataProviderColumnRuleSet.NotNullableWithAutoDefault;

		//4. IsNullable = false, IsUnique = true, AutoDefaultValue = true, DefaultValue = null
		if (!IsNullable && IsUnique && AutoDefaultValue && DefaultValue is null)
			return TfDataProviderColumnRuleSet.UniqueNotNullable;
...
public enum TfDataProviderColumnRuleSet
{
	...
	NotNullableWithCustomDefault = 1,
	//3. IsNullable = false, IsUnique = false, AutoDefaultValue = true, DefaultValue = null
	[Description("required - autogenerated value when not provided")]
	NotNullableWithAutoDefault = 2,
	//4. IsNullable = false, IsUnique = true, AutoDefaultValue = true, DefaultValue = null
